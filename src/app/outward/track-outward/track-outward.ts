...
@Component({
  selector: 'app-track-outward',
  imports: [DynamicForm],
  templateUrl: './track-outward.html',
  styleUrl: './track-outward.css',
})
export class TrackOutward {

  trackOutwardForm!: FormGroup
  trackOutwardfields: DynamicField[] = [
    {
      type: 'text',
      label: 'Inward Number',
  ...
  trackOutward(): void {
    ...
    console.log('form =', this.trackOutwardForm);

    if (this.comSrv.validateForm(this.trackOutwardForm)) {
      console.log(this.trackOutwardForm.getRawValue())
    } else {
      console.log('trackOutwardForm is valid', this.comSrv.validateForm(this.trackOutwardForm));
    }
  }
